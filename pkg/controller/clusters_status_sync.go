...
func addClustersStatusController(mgr ctrl.Manager, transport transport.Transport, syncInterval time.Duration) error {
	...
	return newGenericStatusSyncController(mgr, "clusters-status-sync", transport, finalizerName,
		datatypes.ManagedClustersMsgKey, createObjFunction, syncInterval)
}
